...
namespace _03.JediCodeX
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Regex rgxName = new Regex(Regex.Escape(namePattern) + @"([A-Za-z]{" + namePattern.Length + @"})(?![A-Za-z])");
            Regex rgxMessage = new Regex(Regex.Escape(messagePattern) + @"([A-Za-z0-9]{" + messagePattern.Length + @"})(?![a-zA-Z0-9])");
